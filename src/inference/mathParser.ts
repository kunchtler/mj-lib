// Modified from https://stackoverflow.com/questions/2276021/evaluating-a-string-as-a-mathematical-expression-in-javascript/75355272#75355272
// Posted by tanguy_k
// Retrieved 2026-01-18, License - CC BY-SA 4.0

// Modified the operators and function to use Fraction.js.
// Modified the returned value of the evaluation to be a fraction and not a number.
...
export const operators: {
    ...
    "+": {
        func: (x, y) => new Fraction(x).add(y).toFraction(),
        precedence: 1,
        associativity: "left",
        arity: 2
    },
    "-": {
        func: (x, y) => new Fraction(x).sub(y).toFraction(),
        precedence: 1,
        associativity: "left",
        arity: 2
    },
    "*": {
        func: (x, y) => new Fraction(x).mul(y).toFraction(),
        precedence: 2,
        associativity: "left",
        arity: 2
    },
    "/": {
        func: (x, y) => new Fraction(x).div(y).toFraction(),
        precedence: 2,
        associativity: "left",
        arity: 2
    },
    "%": {
        func: (x, y) => new Fraction(x).mod(y).toFraction(),
        precedence: 2,
        associativity: "left",
        arity: 2
    },
    "^": {
        ...
        // Math.pow(-2, 2) => -4
        func: (x, y) => new Fraction(x).pow(y).toFraction(),
        precedence: 3,
        associativity: "right",
        arity: 2
    }
...
export const functions: {
    ...
    min: { func: (x, y) => (new Fraction(x).lt(y) ? x : y), arity: 2 },
    max: { func: (x, y) => (new Fraction(x).gt(y) ? x : y), arity: 2 }
};
